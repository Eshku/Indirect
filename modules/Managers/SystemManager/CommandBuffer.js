...
export class CommandBuffer {
	 ...
	 * This contains the core recursive logic and is called by both the immediate-mode
	 * `EntityManager.instantiateBatch` and the deferred-mode `_flushBatchInstantiations`.
	 * This centralizes the complex logic in the CommandBuffer, which is responsible for all
	...
	_executeInstantiateBatch(prefabId, count, { overrides = {} } = {}) {
		return this.executor._executeInstantiateBatch(prefabId, count, { overrides })
	}

	/**
	 * The most powerful internal, immediate-mode implementation for batch-instantiating a prefab.
	 * It handles hierarchical creation for multiple entities, each with its own unique override data.
	 * This is the new core logic for all data-driven prefab creation.
	...
	_executeInstantiateWithData(prefabId, overridesArray) {
		return this.executor._executeInstantiateWithData(prefabId, overridesArray)
	}

	_executeCreateEntitiesWithData(creationData) {
		return this.executor._executeCreateEntitiesWithData(creationData)
	}
}
