...
export class AoSArchetypeAccessor {
	constructor(archetypeManager, archetypeId, typeID) {
		...
		const data = archetypeManager.getData(archetypeId)
		this._array = data.componentArrays[typeID]
	}
	get(entityIndex) {
...
export class SoAArchetypeAccessor {
	constructor(archetypeManager, archetypeId, typeID) {
		const componentManager = archetypeManager.componentManager
		const data = archetypeManager.getData(archetypeId)
		const info = componentManager.componentInfo[typeID]
		this._propArrays = data.componentArrays[typeID]
		...
		this._view = new SoAComponentView(info, this._propArrays, componentManager.stringInterningTable)
	}
