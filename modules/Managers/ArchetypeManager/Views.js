 ...
 *
 * - **No Schema (`Cold` component)**:
 *   - **Use Case**: For complex JavaScript objects (`Map`, `Set`), references to external objects (e.g., PIXI DisplayObjects), or data that is accessed infrequently.
 *   - **Example**: `SharedCooldowns: { constructor() { this.cooldowns = new Map() } }`
 *
 * 
 *
...
export class FixedStringView {
	constructor(stringInterningTable, propArrays, prefix, capacity) {
		this._stringInterningTable = stringInterningTable // We borrow its encoder/decoder
		this._propArrays = propArrays
		this._prefix = prefix
	 ...
	 */
	set(value) {
		const encoded = this._stringInterningTable.encoder.encode(value)
	...
	toString() {
		...
		return this._stringInterningTable.decoder.decode(new Uint8Array(bytes))
	}
...
export class InternedStringView {
	constructor(stringInterningTable, offsetArray, lengthArray) {
		this._stringInterningTable = stringInterningTable
		this._offsetArray = offsetArray
	 ...
	 */
	get bytes() {
		return this._lengthArray[this._index]
	}
	 ...
	 */
	equals(targetString) {
		const offset = this._offsetArray[this._index]
		const bytes = this._lengthArray[this._index]
		return this._stringInterningTable.compare(offset, bytes, targetString)
	}
	...
	startsWith(searchString) {
		const offset = this._offsetArray[this._index]
		const bytes = this._lengthArray[this._index]
		return this._stringInterningTable.startsWith(offset, bytes, searchString)
	}
	...
	endsWith(searchString) {
		const offset = this._offsetArray[this._index]
		const bytes = this._lengthArray[this._index]
		return this._stringInterningTable.endsWith(offset, bytes, searchString)
	}
	...
	includes(searchString) {
		const offset = this._offsetArray[this._index]
		const bytes = this._lengthArray[this._index]
		return this._stringInterningTable.includes(offset, bytes, searchString)
	}
	...
	toString() {
		const offset = this._offsetArray[this._index]
		const bytes = this._lengthArray[this._index]
		return this._stringInterningTable.get(offset, bytes)
	}
...
export class SoAComponentView {
	constructor(info, propArrays = {}, stringInterningTable) {
		this._info = info
		this._propArrays = propArrays
		this._stringInterningTable = stringInterningTable
		this._index = -1
		...
		for (const propName of info.originalSchemaKeys) {
			...
			if (rep?.type === 'string') {
				...
				if (!this._internedStringViews[propName]) {
					this._internedStringViews[propName] = new InternedStringView(
						this._stringInterningTable,
						this._propArrays[offsetKey],
						this._propArrays[lengthKey]
					)
				}
				...
				Object.defineProperty(this, propName, {
					...
					set: value => {
						...
						const { offset, length } = this._stringInterningTable.intern(value)
						this._propArrays[offsetKey][this._index] = offset
						this._propArrays[lengthKey][this._index] = length
					},
					enumerable: true,
				})
				...
				Object.defineProperty(this, propName, {
					...
					set: value => {
						...
						if (typeof value === 'string') {
							...
							console.error(
								`Attempted to set invalid value of type '${typeof value}' to enum property '${propName}'`
							)
						}
					},
					enumerable: true,
