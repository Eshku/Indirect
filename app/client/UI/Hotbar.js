...
export class Hotbar {
	...
	getSlotIndexAt(position) {
		// Convert the global position to the container's local coordinates
		const localPoint = this.container.toLocal(position)

		// Check if the point is within the rough vertical bounds of the slots
		if (localPoint.y < 0 || localPoint.y > SLOT_SIZE) {
			return null
		}

		// Calculate the potential slot index based on the x position
		const slotIndex = Math.floor(localPoint.x / (SLOT_SIZE + SLOT_PADDING))

		// Check if the point is in the padding area between slots
		const positionInSlot = localPoint.x % (SLOT_SIZE + SLOT_PADDING)
		if (positionInSlot > SLOT_SIZE) {
			return null // It's in the padding, not on a slot
		}

		// Final check to ensure it's a valid index within our slots array
		if (slotIndex >= 0 && slotIndex < HOTBAR_SLOT_COUNT) {
			return slotIndex
		}

		return null // Outside the horizontal bounds of the hotbar
	}
	...
	swapSlot(newSlotIndex) {
		...
		// Deactivate the current slot and activate the new one.
