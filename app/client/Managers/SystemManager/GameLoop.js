	...
	_loop = async () => {
		...
		while (this.accumulator >= this.FIXED_TIMESTEP) {
			...
			if (logicSystems.length > 0) {
				...
				// This ensures that structural changes made during tick N are applied and visible
				// to reactive systems running in tick N+1, even when multiple logic ticks
				// are processed in a single frame (i.e., during lag catch-up).
				// We flush using the *next* tick's ID to timestamp the changes correctly.
				const cbStartTime = performance.now()
				// The next tick is `this.currentTick + 1` at this point in the loop.
				this.systemManager.commandBuffer.flush(this.currentTick + 1)
		...
		// If the logic loop did not run, but there are commands from input/timed systems,
		// we must flush them now. This ensures the world state is consistent for the visuals group.
		// If the logic loop *did* run, its internal flush already handled this.

		if (!logicLoopRanAtLeastOnce && this.systemManager.commandBuffer.hasCommands()) {
			// We flush using the currentTick, which represents the next tick that will run.
			const cbStartTime = performance.now()
			this.systemManager.commandBuffer.flush(this.currentTick + 1)
			const cbEndTime = performance.now()
			this.workerManager.broadcastDeltas()
